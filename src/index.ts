...
// @ts-ignore
import { MultiSelect, Input } from 'enquirer';
...
async function main() {
  ...
  rl.pause(); // pause readline so enquirer can use stdin

  const prompt = new MultiSelect({
    name: 'environments',
    message: 'Qual(is) ambiente(s) você usa? (Use as setas, espaço para selecionar e enter para confirmar)',
    choices: [
      { name: 'Antigravity', value: '1' },
      { name: 'Codex', value: '2' },
  ...
  if (selectedEnvs.length === 0) {
    console.log('Nenhum ambiente selecionado. Saindo...');
    process.exit(0);
  }
  ...
  const generateAntigravity = selectedEnvs.includes('Antigravity');
  const generateRules = selectedEnvs.includes('Codex') || selectedEnvs.includes('Claude Code');

  const transitionDirective = '\\n\\n**Diretiva de Transição:** Sempre que o seu trabalho for finalizado e for necessário passar a bola para outra persona (ex: de PO para Dev, ou de Dev para Reviewer), **recomende fortemente ao usuário que inicie um novo chat/thread** com a próxima persona. Isso mantém o foco da conversa e preserva o histórico de forma organizada.';
  ...
  console.log('\\nPronto, ' + name + '! Seus agentes foram gerados com sucesso!');
